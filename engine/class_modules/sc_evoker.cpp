...
static std::function<int( actor_target_data_t* )> d_fn( T d, bool stack = true )
{
  ...
  else if constexpr ( std::is_invocable_v<T, evoker_td_t::dots_t> )
  {
    if ( stack )
      return [ d ]( actor_target_data_t* t ) {
        auto dot = std::invoke( d, static_cast<evoker_td_t*>( t )->dots );
...
struct simplified_player_t : public player_t
{
  ...
  void init_items() override
  {
    ...
    std::map<slot_e, std::string> default_items = {
        ...
        { SLOT_FINGER_1,
          fmt::format( ",id=207159,ilevel={},gem_id=213494/213494,enchant=radiant_mastery_3", item_level ) },
        { SLOT_FINGER_2,
          fmt::format( ",id=237570,ilevel={},gem_id=213494/213494,enchant=radiant_mastery_3", item_level ) },
...
struct dracthyr_commando_t : evoker_pet_t
{
  ...
  {
    pyre_t( dracthyr_commando_t* p ) : pet_spell_t( p, "pyre", p->evoker()->talent.scalecommander.pyre_spell_tww3 )
    {
      background = true;
      aoe        = -1;
    }
...
public:
  ...
  size_t available_targets( std::vector<player_t*>& target_list ) const override
  {
    ...
    for ( const auto& t : sim->player_no_pet_list )
    {
      if ( t->is_sleeping() || p()->get_target_data( t )->buffs.ebon_might->up() || t == player ||
           ( t->role == ROLE_TANK || t->role == ROLE_HEAL ) )
        continue;
...
struct eternity_surge_t : public empowered_charge_spell_t
{
  ...
  {
    eternity_surge_damage_t( evoker_t* p, std::string_view name ) : base_t( name, p, p->find_spell( 359077 ) )
    {
    }
...
struct shattering_star_proc_t : evoker_spell_t
{
  ...
  {
    star = p->get_secondary_action<shattering_star_t>( name, name, proc_type );
    if ( star )
    {
      add_child( star );
    }
...
struct dragonrage_t : public evoker_spell_t
{
      ...
      shattering_star( nullptr )
  {
    ...
    {
      shattering_star = p->get_secondary_action<spells::shattering_star_proc_t>(
          "shattering_star_2pc_dragonrage_helper", "shattering_star_2pc_dragonrage", 1 );
...
struct engulf_t : public evoker_spell_t
{
  ...
  {
    engulf_damage_t( evoker_t* p ) : base_t( "engulf_damage", p, p->talent.flameshaper.engulf_damage )
    {
      ...
      {
        replicated_empower_action =
            p->get_secondary_action<fire_breath_traveling_flame_t>( "fire_breath_traveling_flame" );
      }
    }
    ...
    int count_dots( player_t* target ) const override
    {
      ...
      return td->dots.fire_breath->is_ticking() + td->dots.fire_breath_traveling_flame->is_ticking() +
             td->dots.enkindle->is_ticking() + td->dots.living_flame->is_ticking();
    }
  ...
  if ( sets->has_set_bonus( EVOKER_DEVASTATION, TWW2, B2 ) )
  {
    struct devastation_tww2_2pc : public dbc_proc_callback_t
    {
        ...
        : dbc_proc_callback_t( p, e ), damage_spell( nullptr )
      {
        ...
        damage_spell = p->get_secondary_action<spells::shattering_star_proc_t>( "shattering_star_2pc_helper",
                                                                                "shattering_star_2pc", 2 );
      }
    ...
    switch ( mote_buff )
    {
      ...
      case mote_buffs_e::SHIFTING_SANDS:
        if ( !target && allies_with_my_ebon.size() > 0 )
        {
          ...
          auto it = range::partition(
              helper, [ this ]( player_t* t ) { return !get_target_data( t )->buffs.shifting_sands->check(); } );
...
double evoker_t::get_molten_embers_multiplier( player_t* target, bool recalculate ) const
{
  ...
  if ( td && td->dots.fire_breath->is_ticking() )
  {
    ...
    auto firebreath_duration = 20_s + timespan_t::from_seconds( talent.blast_furnace->effectN( 1 ).base_value() ) -
                               ( static_cast<int>( empower ) - 1 ) * 6_s;
