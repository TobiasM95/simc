...
class SimcEnv(gym.Env):
    ...
    def __init__(
        ...
        Args:
            ...
            show_simc_stderr: Print simc stderr lines in real-time (debug mode).
        """
        super().__init__()
        ...
        self._num_resources = 0  # inferred from probe message (resource_pct length)
        ...
        # [gcd_rem_norm, ttd_norm] + resource_pct[num_resources] + cd_charges_f[n_actions]
    ...
    def _probe_action_space(self) -> None:
        ...
        try:
            ...
            # Infer resource vector length from simc output (don't hardcode RESOURCE_MAX)
            resource_pct = msg.get("resource_pct", [])
            if not isinstance(resource_pct, list) or len(resource_pct) == 0:
                raise RuntimeError(
                    "Probe message missing valid resource_pct array."
                )
            self._num_resources = len(resource_pct)
            ...
            # 2 (gcd_rem_norm, ttd_norm) + num_resources (resource_pct) + n_actions (cd_charges)
                ...
                for line in self._process.stderr:
                    with self._stderr_lock:
                        ...
                        if self.show_simc_stderr:
                            # Optional real-time stderr echo for debugging.
    ...
    def _parse_state(
        ...
        # Build observation vector with fixed dimensions
        obs_parts = [
            gcd_rem_norm,
            ttd_norm,
        ]

        # Resource features: enforce fixed length from probe
        resource_pct = msg.get("resource_pct", [])
        if len(resource_pct) < self._num_resources:
            resource_pct = resource_pct + [0.0] * (self._num_resources - len(resource_pct))
        elif len(resource_pct) > self._num_resources:
            resource_pct = resource_pct[: self._num_resources]
        ...
        if len(buff_remains) < self._num_buffs:
            buff_remains = buff_remains + [0.0] * (self._num_buffs - len(buff_remains))
        elif len(buff_remains) > self._num_buffs:
            buff_remains = buff_remains[: self._num_buffs]
        if len(buff_stacks) < self._num_buffs:
            buff_stacks = buff_stacks + [0.0] * (self._num_buffs - len(buff_stacks))
        elif len(buff_stacks) > self._num_buffs:
            buff_stacks = buff_stacks[: self._num_buffs]
        ...
        if len(dot_remains) < self._num_dots:
            dot_remains = dot_remains + [0.0] * (self._num_dots - len(dot_remains))
        elif len(dot_remains) > self._num_dots:
            dot_remains = dot_remains[: self._num_dots]
        if len(dot_stacks) < self._num_dots:
            dot_stacks = dot_stacks + [0.0] * (self._num_dots - len(dot_stacks))
        elif len(dot_stacks) > self._num_dots:
            dot_stacks = dot_stacks[: self._num_dots]
        obs_parts.extend(dot_remains)
        obs_parts.extend(dot_stacks)

        obs = np.array(obs_parts, dtype=np.float32)
        expected_obs_dim = self.observation_space["obs"].shape[0]
        if obs.shape[0] != expected_obs_dim:
            raise RuntimeError(
                f"Observation size mismatch: expected {expected_obs_dim} but got {obs.shape[0]} "
                f"(resources={self._num_resources}, actions={self._num_actions}, "
                f"buffs={self._num_buffs}, dots={self._num_dots})"
            )
...
def run_demo(args):
    ...
    for ep in range(args.episodes):
        ...
        print(f"Spec ID: {info.get('spec_id', 0)}")
        print(f"Buff labels: {info.get('buff_labels', [])}")
        print(f"Dot labels: {info.get('dot_labels', [])}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-blacklist",
        action="store_true",
        help="Disable Python-side action blacklist and keep full action space from simc",
    )
    parser.add_argument(
        "--show-simc-stderr",
        action="store_true",
        help="Echo simc stderr in real-time (useful for debugging, noisy for training)",
    )
